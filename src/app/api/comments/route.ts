...
export async function GET(request: NextRequest) {
  try {
    // Uncomment the following lines if you want to include user verification
    const user = verifyToken(request);
    ...
    return NextResponse.json({ error: 'Failed to fetch comments' }, { status: 500 }); // Return an error response
  } 
}